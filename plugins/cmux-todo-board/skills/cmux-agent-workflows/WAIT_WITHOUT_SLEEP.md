...
- `poll-wait.sh` — production wrapper around the `cmux events` pattern described in this doc.
  Dual-source: event-driven primary with `poll-push.sh` fallback. Use it for the orchestrator
  delegation cycle instead of writing the `cmux events | grep -m1` pipe by hand.
